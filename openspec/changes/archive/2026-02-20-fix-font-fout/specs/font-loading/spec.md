...
### Requirement: MapleMono uses font-display block
MapleMono NF CN 的所有 `@font-face` 声明 SHALL 使用 `font-display: block`，不得使用 `font-display: swap`。

#### Scenario: font-display is block
- **WHEN** `public/fonts.css` 中 MapleMono NF CN 的 `@font-face` 被解析
- **THEN** 每条声明的 `font-display` 值为 `block`
